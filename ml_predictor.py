...
# Попытка подключить PyTorch для ускорения на GPU.
...
class MLPredictor:
    ...
    будет обучаться на GPU Apple Silicon (через ``mps``) или CUDA, что заметно
    ...
    def train_model(
        ...
        данные переводятся на выбранное устройство (GPU/CPU), создаётся простая
        нейронная сеть с одним линейным слоем, и выполняется несколько эпох
        градиентного спуска. В противном случае используется логистическая
        регрессия из scikit‑learn."""
