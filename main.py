...
def ensure_apple_silicon() -> None:
    """Проверить, что код выполняется на macOS с архитектурой ARM64.

    Raises
    ------
    EnvironmentError
        Если текущая платформа не является Apple Silicon.
    """

    system = platform.system()
    machine = platform.machine().lower()
    if system != "Darwin" or not machine.startswith("arm"):
        raise EnvironmentError(
            "This program requires macOS on Apple Silicon (ARM64)."
        )
