"""Простой высокоуровневый торговый бот (асинхронный)."""
...
class TradingBot:
    ...
    executor: ThreadPoolExecutor = field(default_factory=lambda: ThreadPoolExecutor(max_workers=3))
    ...
    async def _place_with_retry(
        ...
        for attempt in range(self.max_retries):
            try:
                order = await self.exchange.place_order(pair, adj_price, qty, side, exchange=exchange)
                order_id = order.get("id") or order.get("order_id") or order.get("result", {}).get("orderId")
                if order_id is None:
                    raise RuntimeError("No order id returned")
                status = await self.exchange.get_order_status(order_id, pair, exchange)
                state = status.get("status") or status.get("result", {}).get("orderStatus")
                if state and str(state).lower() in {"filled", "closed"}:
                    order.update({"pair": pair, "price": adj_price, "qty": qty, "side": side, "fee": fee})
                    return order
    ...
    async def execute_trade(
        ...
        try:
            ...
            if self.strategy_manager:
                await self.strategy_manager.evaluate_market(self.exchange, spot_pair, exchange)
                active_strategy = self.strategy_manager.get_active_strategy()
            else:
                active_strategy = self.strategy

            if active_strategy is None:
                handle_error("No active strategy", ValueError("strategy is None"))
                return {}

            decision_task = active_strategy.apply_strategy(self.exchange, spot_pair, futures_pair)
            balance_task = self.exchange.check_balance(exchange)
            decision, balance = await asyncio.gather(decision_task, balance_task)

            price = float(decision["spot_price"])
            ...
            if self.risk_manager:
                ...
                open_pos = len(self.position_manager.positions) if self.position_manager else 0
            ...
            # Аномалии считаем отдельно, чтобы не смешивать типы результатов gather
            anomalies: List[int] = []
            if len(self.price_history) >= 5:
                anomalies = await self._detect_anomalies_async()

            # Параллельно отменяем ордера
            ...
            if signal == 1:
                spot_coro = self._place_with_retry(spot_pair, decision["spot_price"], qty, "Buy", exchange)
                fut_coro = self._place_with_retry(futures_pair, decision["futures_price"], qty, "Sell", exchange)
                orders["spot"], orders["futures"] = await asyncio.gather(spot_coro, fut_coro)
            elif signal == -1:
                spot_coro = self._place_with_retry(spot_pair, decision["spot_price"], qty, "Sell", exchange)
                fut_coro = self._place_with_retry(futures_pair, decision["futures_price"], qty, "Buy", exchange)
                orders["spot"], orders["futures"] = await asyncio.gather(spot_coro, fut_coro)

            if orders and self.position_manager:
                for order in orders.values():
                    side = "long" if str(order["side"]).lower() == "buy" else "short"
                    pair = str(order["pair"])
                    existing = self.position_manager.positions.get(pair)
                    if existing and existing.side != side:
                        self.position_manager.close_position(pair, float(order["price"]))
                    self.position_manager.open_position(pair, float(order["qty"]), float(order["price"]), side)
    ...
    def update_trading_pairs(self, pairs: List[Tuple[str, str, str]]) -> None:
        ...
        for exch, spot, fut in added:
            self.notifier.log_notification("pairs", f"added {exch}:{spot}:{fut}")
        for exch, spot, fut in removed:
            self.notifier.log_notification("pairs", f"removed {exch}:{spot}:{fut}")
